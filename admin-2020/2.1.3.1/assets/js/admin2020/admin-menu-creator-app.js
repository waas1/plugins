...
const a2020menuCreatorItems = JSON.parse(a2020_menucreator_ajax.menuItems);
...
const a2020menuCreatorOptions = {
  data() {
    return {
      ...
      master: {
        menuItems: a2020menuCreatorItems,
        searchString: "",
      },
      ui: {
        activeTab: "items",
  ...
  methods: {
    exportMenu(themenu) {
      ...
      var dlAnchorElem = document.getElementById("uipress_export_menus");
    ...
    import_menu() {
      ...
      reader.onload = function (evt) {
        ...
        if (parsed != null) {
          ...
          UIkit.notification("Menu imported", { pos: "bottom-left", status: "success" });
          self.saveSettings();
        } else {
    ...
    confirmDelete(themenu) {
      self = this;
      UIkit.modal.confirm("Are you sure you want to delete this menu?").then(
        function () {
    ...
    editMenuItem(item) {
      ...
      UIkit.offcanvas("#edit-menu-item").show();
    },
    ...
    thisSearchInput: function (newValue, oldValue) {
      ...
      if (newValue.length > 0) {
        jQuery.ajax({
          ...
          type: "post",
          data: {
            action: "uipress_get_users_and_roles",
            security: a2020_menucreator_ajax.security,
...
a2020menuCreator.component("icon-select", {
  ...
  template:
    '<div class="uk-dropdown wpteams-no-after a2020-available-container uk-width-medium" uk-dropdown="pos:bottom-center;mode:click;offset:10;">\
    <div class="uk-inline uk-width-1-1 wpteams-border uk-margin-small-bottom">\
    <span class="uk-form-icon" uk-icon="icon: search" style="	left: -8px;"></span>\
    <input class="uk-input uk-search-input " type="text" style="background: none;border:none;" \
    placeholder="Search Icons" v-model="thisSearchInput" autofocus>\
    </div>\
    <div class="">\
    <template v-for="option in allIcons">\
     <a class="uk-link-muted" @click="chosenicon(option)">\
     <span class="material-icons-outlined" style="padding:10px;">\
     {{option}}\
     </span>\
     </a>\
    </template>\
    <div>\
    <button @click="removeIcon()" class="uk-button uk-button-danger uk-button-small" type="button">Clear Icon</button>\
    </div>\
    </div>\
  </div>\
    ',
