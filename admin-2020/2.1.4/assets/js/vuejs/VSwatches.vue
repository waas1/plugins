<template>
  <div class="vue-swatches" tabindex="-1" @blur="(e) => onBlur(e.relatedTarget)">
    ...
    <div v-if="!inline" ref="triggerWrapper" class="vue-swatches__trigger__wrapper" @click="togglePopover">
      <slot name="trigger">
        <div
          ...
          :style="triggerStyles"
        >
          <div v-show="isNoColor" class="vue-swatches__diagonal__wrapper vue-swatches--has-children-centered">
    ...
    <transition name="vue-swatches-show-hide">
      ...
      <div ref="containerWrapper" v-show="inline || isOpen" class="vue-swatches__container" :class="{ 'vue-swatches--inline': inline }" :style="containerStyles">
        <!-- The wrapper handles the internal spacing -->
        <div class="vue-swatches__wrapper" :style="wrapperStyles">
          <!-- for nested distribution -->
          <template v-if="isNested">
            <div v-for="(swatchRow, index) in computedSwatches" :key="index" class="vue-swatches__row">
              <v-swatch
                v-for="(swatch, swatchIndex) in swatchRow"
                :key="swatchIndex"
                :is-last="index === computedSwatches.length - 1 && swatchIndex === swatchRow.length"
                :row-length-setted="rowLength !== null || presetRowLength !== null"
        ...
        <div v-if="showFallback" class="vue-swatches__fallback__wrapper" :style="computedFallbackWrapperStyles">
          ...
          <button v-if="showFallbackOk" class="vue-swatches__fallback__button" :class="fallbackOkClass" @click.prevent="onFallbackButtonClick">
            {{ fallbackOkText }}
          </button>
        </div>
      </div>
...
export const extractPropertyFromPreset = (presetName, property, alwaysReturn) => {
  if (typeof presetName !== "string") return null;
  else if (presetName === "text-basic") return textBasicPreset[property] === undefined ? null : textBasicPreset[property];
  else if (presetName === "text-advanced") return textAdvancedPreset[property] === undefined ? null : textAdvancedPreset[property];
  else if (presetName === "basic") return basicPreset[property] === undefined ? null : basicPreset[property];
  else if (alwaysReturn) return basicPreset[property] === undefined ? null : basicPreset[property];
  else return null;
};
export default {
  name: "v-swatches",
  ...
  computed: {
    ...
    computedRowLength() {
      ...
      else if (this.computedSwatches.length < DEFAULT_ROW_LENGTH && !this.isNested) return this.computedSwatches.length;
  ...
  methods: {
    ...
    getAlwaysOnScreenStyle() {
      ...
      const leftMin = 5;
      const rightMax = (document.documentElement.clientWidth || window.innerWidth) - 5;
      const topMin = 5;
      const bottomMax = (document.documentElement.clientHeight || window.innerHeight) - 5;
      ...
      if (this.popoverY === "top") {
        if (triggerRect.top - containerRect.height < topMin) {
          // Showing bellow
          styles.top = `${triggerRect.height + DEFAULT_TRIGGER_CONTAINER_SPACE}px`;
          styles.bottom = "auto";
        } else {
          // Showing above
          styles.bottom = `${triggerRect.height + DEFAULT_TRIGGER_CONTAINER_SPACE}px`;
          styles.top = "auto";
        }
      } else {
        /* istanbul ignore else */
    ...
    getSwatchShowBorder(swatch) {
      ...
      else if (typeof swatch === "object") return swatch.showBorder !== undefined ? swatch.showBorder : this.computedShowBorder;
    },
    ...
    getSwatchDisabled(swatch) {
      ...
      else if (typeof swatch === "object") return swatch.disabled !== undefined ? swatch.disabled : this.disabled;
    },
    ...
    getSwatchAlt(swatch) {
      if (typeof swatch === "string") return swatch;
      else if (typeof swatch === "object") return swatch.alt || this.getSwatchLabel(swatch);
    },
